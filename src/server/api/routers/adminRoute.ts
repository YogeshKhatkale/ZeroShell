					...
					if (fs.existsSync(ztBackupPath)) {
						...
						if (dockerMode) {
							...
							try {
								...
								console.warn(
									"Could not start ZeroShell container automatically. Please run: docker restart zeroshell",
								);
							}
						} else {
							...
							if (!serviceStarted) {
								console.warn(
									"Could not start ZeroShell service automatically. Please run: systemctl start zeroshell-one",
								);
							}
						}
					}
